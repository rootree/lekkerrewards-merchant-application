...
public class CheckInByEmailJob extends Job {

    private CheckInByEmailRequest request;
    //private LekkerRequest request;

    public CheckInByEmailJob(CheckInByEmailRequest request) {
        // This job requires network connectivity,
        // and should be persisted in case the application exits before job is completed.
        super(new Params(Priority.MID).requireNetwork().persist().groupBy("send_request").delayInMs(1000));//order of 1 matter, we don't want to send two in parallel
    ...
    public void onAdded() {
       ...
       // call = APIService.checkInByQR(request);
        //EventBus.getDefault().post(new SendingRequestEvent(email));
    }
